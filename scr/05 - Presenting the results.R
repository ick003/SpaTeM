...
plotSPTM(ResGibbs, "hyperparameter", keepRun = 10000:15000)
...
ggplot(data = predData, aes(x = date, y = obs, col = ID)) + geom_point(size=0.5) + geom_line(size = 0.5) + 
  geom_line(aes(x = date, y = yP, group = ID), col = "black") + facet_wrap(~ID, scales = "free") + 
...
Cldf = data.frame(ID = rownames(pi0),Cluster = apply(t(apply(ResGibbsM$GibbsOut$theta$zH[1000:1500,,,1],c(2,3),mean)),1,which.max))
...
predData$yPp =  colMeans(predictSPTM(ResGibbsM, keepRun = 1500:2500)$YpwN)
Cldf = data.frame(ID = rownames(pi0),Cluster = apply(t(apply(ResGibbsMS$GibbsOut$theta$zH[3500:5000,,,1],c(2,3),mean)),1,which.max))
